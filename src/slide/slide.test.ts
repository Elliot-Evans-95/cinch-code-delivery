...
describe('Given a new version of the Slide class has been made', () => {
    ...
    beforeAll(
        () =>
            (dom = new Slide(
                RouterHelper.retrieveActiveRoute(new Router(fakeRoutes)),
                appShell,
                pageShell,
                styleShell,
                documentWrapper,
    ...
    describe('when "add" is called', () => {
        ...
        test('then the title of the tab is changed', () => {
            ...
            expect((dom as any)._document.document.title).toEqual(
                fakeRoute.routeName
            )
        })
        ...
        test('then the "pageShell" has content', () => {
            ...
            expect((dom as any)._pageShell.element.innerHTML).toEqual(
                fakeRoute.routeHTML
            )
        })
        ...
        test('then the "appShell" has content', () => {
            ...
            expect((dom as any)._appShell.element.textContent).toEqual(
                'Page one of presentation#app-shell {background: aqua;}'
            )
        })
        ...
        test('then the "styleShell" has content', () => {
            ...
            expect((dom as any)._styleShell.element.textContent).toEqual(
                '#app-shell {background: aqua;}'
            )
        })

        test('then a message is published when adding content to the dom', () => {
            const publishSpy = jest.spyOn(messageEvents, 'publish')
