...
import { setActiveRootStorage } from '../storage/storage'
...
export class Slide implements Dom {
    ...
    constructor(
        ...
        this.remove()
        this.add(route)
        if (!isNotes) setActiveRootStorage(route)
    }

    remove(): void {
        while (this._appShell.element.firstChild) {
            this._appShell.element.removeChild(
                this._appShell.element.firstChild
            )
        }
